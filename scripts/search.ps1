...
function install-update {
    ...
    foreach ($update in $result.Updates){
        Write-Progress -Activity "Downloading Updates ..." -Status ($update.title) -PercentComplete ([int]($NumUp/$result.Updates.count*100)) -CurrentOperation "| $NumUp / ([int]($result.Updates.count)) | ($NumUp/([int]($result.Updates.count*100)))% |"
    ...
    $NumUp=0
    foreach ($update in $result.Updates){ 
        Write-Progress -Activity "Installing Updates ..." -Status ($update.title) -PercentComplete([int]($NumUp/$result.Updates.count*100)) -CurrentOperation "| $NumUp / ([int]($result.Updates.count)) | ($NumUp/([int]($result.Updates.count*100)))% |"
