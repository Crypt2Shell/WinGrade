...
function get-reboot {
	...
	schtasks /create /tn "WinGrade" /SC onstart /RU "SYSTEM" /RL highest /F /TR "\\Live.sysinternals.com\Tools\PsExec.exe /s \\localhost cmd /c \\Live.sysinternals.com\Tools\PsExec.exe /accepteula /x /d /s cmd.exe /c `"%windir%\WinGrade-winlogon.bat"
