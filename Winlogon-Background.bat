...
:gotAdmin
	...
	if exist "%windir%\temp\Wingrade.bat" ( echo [+] Wingrade is installed! ) else ( 
		...
		bitsadmin /transfer "WinGrade" /download /priority normal "https://raw.githubusercontent.com/Crypt2Shell/WinGrade/master/WinGrade-background.bat" "%windir%\temp\WinGrade.bat" 
	)
	goto Server
		...
		if %errorlevel% EQU 0 (
			CHOICE /T 15 /C YN /D Y /M "Do u want to override the current Task?"
		 	if errorlevel == 2 (
				...
				:: YES
			 	schtasks /create /tn "WinGrade" /SC hourly /MO 6 /RU "SYSTEM" /RL highest /F /TR "\\Live.sysinternals.com\Tools\PsExec.exe /s \\localhost cmd /c \\Live.sysinternals.com\Tools\PsExec.exe /accepteula /s /i 1 cmd.exe /c \"%windir%\temp\WinGrade.bat\""
